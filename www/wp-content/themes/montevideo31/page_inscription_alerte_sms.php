...
if( !empty($_POST['bew_user_adu_user_mobile']) ) {
	...
	if( is_user_logged_in() ) {
		get_currentuserinfo();	
		$previous = get_user_meta($current_user->ID, 'adu_user_mobile', true);
		$state = update_user_meta( $current_user->ID, 'adu_user_mobile', $tel_nb, $previous );
		if( $state ){
			$saved = true;
			delete_user_meta( $current_user->ID, 'unsubscribed_sms');
	...
	get_currentuserinfo();	
	$previous = get_user_meta($current_user->ID, 'adu_user_mobile', true);
	$state = update_user_meta( $current_user->ID, 'adu_user_mobile', $_SESSION['alert_sms'], $previous );
	unset($_SESSION['alert_sms']);
	if( $state ){
		$saved = true;
		delete_user_meta( $current_user->ID, 'unsubscribed_sms');
	}else{
...
<div id="container">
	<div id="content" role="main">
		...
		if( $state ){
			...
			$retour = bew_send_a_sms( get_user_meta($current_user->ID, 'adu_user_mobile', true) );
		}
		...
		if( $errorMessage ){
			?>
			<div class="frontend_publish_error">
				Vous n'avez pas le droit d'acc&#233;der directement cette page.
			</div>
			<?php
		}
		?>
