...
add_action('widgets_init', 'register_BEW_Widget_RSS');

function register_BEW_Widget_RSS() { register_widget('BEW_Widget_RSS'); }
 ...
 * @since 2.8.0
 */
class BEW_Widget_RSS extends WP_Widget {

	function BEW_Widget_RSS() {
		$widget_ops = array( 'description' => __('Entries from any RSS or Atom feed') );
		$control_ops = array( 'width' => 400, 'height' => 200 );
		$this->WP_Widget( 'rss', __('BEW RSS'), $widget_ops, $control_ops );
	}
	...
	function widget($args, $instance) {
		...
		bew_widget_rss_output( $rss, $instance );
		echo $after_widget;
	...
	function update($new_instance, $old_instance) {
		...
		return bew_widget_rss_process( $new_instance, $testurl );
	}
 ...
 */
function bew_widget_rss_output( $rss, $args = array() ) {
	if ( is_string( $rss ) ) {
		$rss = fetch_feed($rss);
 ...
 */
function bew_widget_rss_form( $args, $inputs = null ) {
