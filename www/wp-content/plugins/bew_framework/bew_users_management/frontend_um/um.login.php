		...
		function validate(){
			...
			if ( count( $_POST ) && $this->form->validate() ){
					...
					if ( !$secure_cookie && is_ssl() && force_ssl_login() && !force_ssl_admin() && ( 0 !== strpos($redirect_to, 'https') ) && ( 0 === strpos($redirect_to, 'http') ) )
						$secure_cookie = false;
					if(!is_email($_POST['user_email'])) return $this->errors[] = 'Votre email est invalide. (exemple: email@exemple.com)';
						...
						if(isset($user_info->user_login)){
							...
							if ( !is_wp_error($user) && !$reauth) {
								...
								if( isset( $_SESSION['product_id'] ) ){
									...
									$this->messages[] = '<strong>Vous allez &#234;tre redirig&#233; vers la page de sauvegarde de votre numéro aux Alertes SMS.'.bew_redirect_js( $bew_p_redirect , 3).'</strong>';
									return;

									
								}else{
									// normal redirection
									$this->messages[] = '<strong>'.__('Vous vous êtes connecté au site avec succès.').' '.get_bloginfo('name').'.</strong>'.(isset($_REQUEST['redirect']) && !empty($_REQUEST['redirect']) ? bew_redirect_js($_REQUEST['redirect']) : bew_redirect_js(get_bloginfo('url')));
									//bew_message('Vous êtes maintenant connecté.', 'message');
				...
				$this->errors[] = sprintf(__('<strong>ERROR</strong>: Mauvaise combinaison Email / Mot de passé. <a href="%s" title="Password Lost and Found">Mot de passe perdu</a>?'), site_url('wp-login.php?action=lostpassword', 'login'));
