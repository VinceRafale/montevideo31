...
function onActionSaveRoutine() {
	...
	$stylesheet = base64decode(getPost("stylesheet"));
	$file = base64decode(getPost("file"));

   // WHITE SPACES BUGFIX

	if(!strlen($stylesheet)) {

		  $stylesheet = base64decode(str_replace(' ', '+', getPost("stylesheet")));
	  	$file = base64decode(str_replace(' ', '+', getPost("file")));

	}
	
	// AVOID STYLESHEET ERASING
	
	if(!strlen($stylesheet)) return false;
...
function debug($message){
	...
	file_put_contents('firefile_debug.txt', "\r\n ".date()." : ".$message, FILE_APPEND);
