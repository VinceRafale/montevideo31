...
default:
	...
	if ( !is_wp_error($user) && !$reauth ) {
		 ...
		 */
		$isUserUnsubscribed = get_user_meta($user->ID, 'unsubscribed_site');
		
		if( $isUserUnsubscribed ){
			// User has unsubscribed
			$errors->add('unregistered', __('Vous vous &#234;tes d&#233;sinscrit du site.'), 'message');
			//$errors = sprintf(__('<strong>Erreur :</strong> Vous vous êtes déjà désinscrits du site.'));
