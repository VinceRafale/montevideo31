<?php
	...
	function bew_users_acl_metabox(){
		...
		$groups = array('_adu_acl_public_access' => array(
		...
		foreach ($groups as $id => $options){
				...
				if((is_array($post_groups) && in_array($id, $post_groups)) || (!is_array($post_groups) && $id == '_adu_acl_public_access')) $checked = " checked='checked' "; else $checked = "";
	...
	function bew_users_acl_metaboxes(){
		...
		add_meta_box( 'user_groups_acl', __('User groups allowed to see content'), 'bew_users_acl_metabox', 'post', 'normal');
