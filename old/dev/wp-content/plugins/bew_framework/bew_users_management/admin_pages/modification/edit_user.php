...
function adu_edit_user_groups($profileuser){
	...
	if(!current_user_can('edit_user', $user_id) || !current_user_can('edit_users')) return;
	...
	foreach ($adu->groups as $id => $options){
		...
		if((is_array($user_groups) && in_array($id, $user_groups)) || $id == 'all_users') $checked = " checked='checked' "; else $checked = "";
	
		
		if($id == 'all_users') $disabled = " disabled='disabled' "; else $disabled = "";
		
		if($id == 'all_users') $groups_html.="<input type='hidden' name='adu_groups[]' value='$id'/>";
	...
	foreach ($adu_fields as $id => $label){
		...
		<td><input type='text' class='regular-text' value='$value' id='adu_$id' name='adu_$id'>
			</td>
		</tr>";
