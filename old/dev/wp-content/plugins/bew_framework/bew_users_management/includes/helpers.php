...
class adu_groups{
	...
	function group_exists($group_id_or_name){
		...
		if( isset($this->groups[$group_id_or_name])) return true;
		
		foreach($this->groups as $group){ if($group_id_or_name == $group['name']) return true; }
	...
	function create($group_name, $group_id = false){
		...
		if($group_id){
			...
			if($this->group_exists($group_id)) return new WP_Error('adu_group_id_already_exists', __('Il y a déja un groupe avec cet identifiant.')); 
		...
		if($this->group_exists($group_name)) return new WP_Error('adu_group_name_already_exists', __('Il y a déja un groupe portant ce nom.')); 
	...
	function save_groups(){
		...
		if(get_option('adu_groups') === $this->groups) return true;
			
		else return new WP_Error('adu_groups_save_failed', __('User Groups save failed.'));		
	...
	function group_name($group_id){
		...
		if(isset($this->groups[$group_id])) return $this->groups[$group_id]['name'];
