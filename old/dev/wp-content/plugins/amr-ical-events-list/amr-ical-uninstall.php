...
/* -------------------------------------------------------------------------------------------------------------*/
	...
	{	
		...
		if (isset ($_POST['reallyuninstall']))  { 
				amr_ical_uninstall();
					echo '<p>'
					.__('Note: Navigating to "Manage AmR ICal Settings" will RELOAD default options - negating the uninstall.', 'amr_ical_list_lang')
					.'</p>';
					echo '<a href="'.'../wp-admin/plugins.php">'.__('Continue to Plugin list to delete files as well','amr_ical_list_lang').'</a>'; 
				}
		...
		if (isset ($_POST['uninstall'])) {
			...
			?><fieldset id="submit">
					...
					<input type="submit" name="cancel" value="<?php _e('Cancel', 'amr_ical_list_lang') ?>" />
					<input type="submit" name="reallyuninstall" value="<?php _e('Really Uninstall Options?', 'amr_ical_list_lang') ?>" />		
				</fieldset>
			</form>
			</div><?php 
		}
