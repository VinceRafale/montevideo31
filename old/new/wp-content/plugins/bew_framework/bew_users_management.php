...
function bew_frontend_um_the_content($content){
	...
	if(stristr($content, '[bew-profile]') ){
		...
		<script type='text/javascript'>
			$(function(){
				$('.um_form #input').submit(function(){
				 var regex = /^([0-9]{9})$/
				 var element = $('#bew_user_adu_user_mobile').attr('value');
				 var result =  regex.test(element);
				 if(result){
				 	$('#bew_user_adu_user_mobile').css({'border':'1px black solid','color':'black'})
				 	return true 
				 }
				 else{
				 	$('#bew_user_adu_user_mobile').css({'border':'1px red solid','color':'red'})
				 	return false
				 }
...
function bew_unsubscribe_form(){
	...
	*/
	...
	$output .= "			
			...
			<input type=\"submit\" name=\"submit\" class=\"button-primary\" value=\"Appliquer les modifications\" onclick=\"confirmation()\">  
    		</form>  			
		</fieldset>
	";
...
function bew_retrieve_user_annonces(){
	...
	while ( have_posts() ) : the_post();
		...
		// different button action regarding the post's status
		if( (get_post_status($p_id) == 'published')||(get_post_status($p_id) == 'pending')){
			$action = "Archiver";
		}elseif( get_post_status($p_id) == 'draft' ){
			$action = "Archiver";
		}else{
			$action = "/ problème /";
		}
		// display button
		$userContent .= "<td>
					<form name=\"ArchivePetiteAnnonce\" action=\"#\" method=\"POST\">
    					<input type=\"hidden\" name=\"archive_annonce_id\" value=\"".$p_id."\" >
    					<input type=\"hidden\" name=\"archive_annonce_action\" value=\"".$action."\" >
    					<input type=\"hidden\" name=\"archive_annonce_user_id\" value=".$current_user->ID." >
    					<input type=\"hidden\" name=\"archive_annonce_title\" value=\"".$p_title."\" >
    					<input type=\"hidden\" name=\"archive_annonce_price\" value=".$custom_fields['annonce_prix']['0']." >
    					<input type=\"hidden\" name=\"archive_annonce_content\" value=\"".$p_content."\" >
    				<input type=\"submit\" name=\"submit\" class=\"button-primary\" value=\"".$action."\">
					</form>
				</td>";
		// close the row
		$userContent .= "</tr>";
