...
class bewBookingsHelper{
		...
		function book($event_id, $user_id, $metadata){
			...
			if($result) 
				return bew_admin_message('This event is already booked for this user.', 'error');
			...
			bew_message('La r&#233;servation a &#233;ŽchouŽ, merci de contacter l\'administrateur du site.', 'error');
			return false;
		}
		...
		function unbook($ids = array(), $args = array()){  //array of booking id or event_id and user_id
			...
			foreach ($ids as $booking_id){
				...
				if(!$wpdb->update( $this->table, $data, $where, $format, $where_format )) 
					$success = false;
			}
			return $success;
...
function bew_cancel_event_booking(){
	...
	if(isset($_REQUEST['bew_cancel_booking'])) 
		$bew->unbook($_REQUEST['bew_cancel_booking']);
}
...
function bew_book_event(){
	...
	if(isset($_REQUEST['bew_book_user_email'])){
		...
		if(!isset($user->ID)) 
			return bew_admin_message('This email address was not found in the database. User does not exist, or email address is incorrect.', 'error');
		else 
			$user_id = $user->ID;
	
	}
	
	if(isset($_REQUEST['bew_book_user_id'])) 
		$user_id = $_REQUEST['bew_book_user_id'];
	
	if(isset($_REQUEST['bew_book_comments'])) 
		$metadata['Comments'] = $_REQUEST['bew_book_comments'];
	
	$bew = new bewBookingsHelper();
	
	if($bew->book($event_id, $user_id, $metadata)) 
		bew_admin_message('Booking done.', 'message');
}
...
function bew_book_single_event(){
	...
	if($wp_query->post_count == 1 && $wp_query->post->ID > 0) {
	  ...
	  if($_REQUEST['bwbook'] == 'yes' && $bew->book($wp_query->post->ID, $current_user->ID , array() )){
	    echo "'im registering a new booking";
	    bew_message('Your booking has been taken into account.', 'message');
	  } 
	
	  if($_REQUEST['bwbook'] == 'no' && $bew->unbook(array(), array('event_id' => $wp_query->post->ID, 'user_id' =>$current_user->ID ))) 
	    bew_message('Your booking has been canceled.', 'message');
	
	  elseif($_REQUEST['bwbook'] == 'no')  
	      bew_message('Error canceling your booking, please contact site administration.', 'error');
